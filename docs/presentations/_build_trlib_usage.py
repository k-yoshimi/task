...
# ---------------------------------------------------------------------------
# 配色 (役割ごとに色分け; _build_pptx.py と同じパレット)
# ---------------------------------------------------------------------------
COLOR_FORTRAN = RGBColor(0x1F, 0x4E, 0x79)
COLOR_C_ABI = RGBColor(0x2E, 0x7D, 0x32)
COLOR_PYTHON = RGBColor(0xEF, 0x6C, 0x00)
COLOR_MCP = RGBColor(0x6A, 0x1B, 0x9A)
COLOR_TOML = RGBColor(0x00, 0x69, 0x7C)
COLOR_TEST = RGBColor(0xC6, 0x28, 0x28)
...
def add_bullets(slide, left, top, width, height, bullets, *, font_size: int = 16):
    box = slide.shapes.add_textbox(left, top, width, height)
    tf = box.text_frame
    tf.word_wrap = True
    tf.margin_left = Emu(36000)
