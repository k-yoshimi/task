...
def handle_run_and_get_state(
    ...
    :class:`Tilib` handle before opening a fresh one so this call is
    ...
    mesh sizes, NSMAX, MODEL_BND, etc.) into the one-shot run because
    libtiapi.so holds singleton Fortran state -- the user-facing
    ...
    """
    try:
        # Force a fresh handle: STATE.close() finalizes any prior Tilib,
