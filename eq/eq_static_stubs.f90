...
! standalone eq Fortran binary path.
!
! eq/Makefile's `eq` target links the full TASK archive chain (libgrf.a
! + libeq.a + libpl.a + ...), which transitively references ~100 GSAF
! graphics primitives (GSCALE, GFRAME, GPLOTP, CONTF*, ...) that the
! per-module *_graphics_stubs.f90 files only partially cover. With CI's
! GFLIBS= (no graphics linkage), those references go unresolved and the
! standalone `eq` binary fails to link.
!
